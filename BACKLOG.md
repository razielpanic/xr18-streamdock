...
| XD-T013 | Clean up orphaned legacy-UUID profile bindings  | P4 |         | Two Stream Dock profiles still hold action bindings against the legacy UUIDs `com.youshriek.xr18fx` (21 bindings) and `com.youshriek.xr18channel` (9 bindings) from before the manifest rebrand to `com.youshriek.xrdock.control.knob` / `com.youshriek.xrdock.channel.button`. Cosmetic cruft only — the current manifest no longer declares those action types, so Stream Dock silently ignores those bindings. To clean up: open Stream Dock, switch to each affected profile, delete the blank/orphaned buttons, and either rebuild them with the current xrDock actions or leave the slots empty. Affected profiles live at `~/Library/Application Support/HotSpot/StreamDock/profiles/08BLVO9X-MDX5-KX6P-30X0-K34RI47KUF35.sdProfile` and `~/Library/Application Support/HotSpot/StreamDock/profiles/9YX0B2Z2-LBO4-0O8X-3607-224WZVQ8UD00.sdProfile` (the `.bak` siblings are Stream Dock's own rotating backups; they'll cycle out naturally once the live manifests are updated). Background: the v0.3.0 commit `ceed257` renamed the JS source files to `xrDock.js` but left the manifest action UUIDs on their legacy values until commit `7be0873` (2026-04-10). Bindings made between those two dates are the orphans. No user-facing impact; file whenever the profile clutter becomes annoying. |
| XD-T014 | Dev / live worktree split for plugin deploy     | P3 |         | Replace the single-tree + symlink setup with a two-tree `git worktree` layout so editor/git state can't leak into production Stream Dock. Dev tree stays at `~/Projects/StreamDock/com.youshriek.xr18fx.sdPlugin/`, new live tree at `~/Projects/StreamDock/xrdock-live/` pinned to branch `live`, Stream Dock plugin symlink repointed at the live tree. Deploy becomes `git -C ~/Projects/StreamDock/xrdock-live merge --ff-only main`; rollback is `git reset --hard <tag>` against the live tree. Both trees share one `.git` — no duplication. Eliminates the class of accidents where `git stash`, branch-switching, or half-saved files expose Stream Dock to broken state (discovered during 2026-04-10 identity cleanup, nothing broke but the window existed). Full plan + step-by-step setup + post-flight checks: `docs/dev-live-worktree-plan.md`. |
